...
namespace MultiserviciosPiscinas.Controllers
{
    ...
    public class ServiciosTecnicosController(PiscinasYMultiserviciosContext context) : Controller
    {
        ...
        public async Task<IActionResult> CambiarTecnico(int id)
        {
            ...
            if (servicio.Estado == "Cerrado" || servicio.Estado == "Cancelado")
            {
                TempData["MensajeError"] = "No se puede cambiar el técnico de un servicio cerrado o cancelado.";
                return RedirectToAction(nameof(Detalle), new { id = servicio.Id });
            }
        ...
        public async Task<IActionResult> CambiarTecnico(CambiarTecnicoServicioViewModel modelo)
        {
            ...
            // <!-- Validar que el técnico exista y sea válido -->
            ...
            {
                ModelState.AddModelError(nameof(modelo.TecnicoId), "El técnico seleccionado no existe, no es válido o no está activo.");
            }
            ...
            if (ModelState.IsValid && servicio.Cita.TecnicoId != modelo.TecnicoId)
            {
                ...
                if (conflicto)
                {
                    ModelState.AddModelError(nameof(modelo.TecnicoId), "El técnico seleccionado ya tiene otra cita programada para esa fecha y hora.");
                }
            }
            ...
            // <!-- Si el técnico es el mismo, no hacer nada -->
            if (servicio.Cita.TecnicoId == modelo.TecnicoId)
            {
                TempData["MensajeExito"] = "El técnico seleccionado es el mismo que ya estaba asignado.";
                return RedirectToAction(nameof(Detalle), new { id = servicio.Id });
            }
            ...
            _context.BitacoraAuditoria.Add(new BitacoraAuditoria
            {
                ...
                ValorAnterior = $"Técnico anterior: {tecnicoAnterior.Nombre} {tecnicoAnterior.ApellidoPaterno}",
                ValorNuevo = $"Técnico nuevo: {tecnicoNuevo!.Nombre} {tecnicoNuevo.ApellidoPaterno}",
                FechaHora = ahora
            });
            ...
            _context.Notificacion.Add(new Notificacion
            {
                ...
                Mensaje = $"Se te asignó el servicio #{servicio.Id} el {servicio.Cita.FechaHora:dd/MM/yyyy} a las {servicio.Cita.FechaHora:HH:mm}.",
                Leida = false,
                FechaCreacion = ahora
            });
            ...
            TempData["MensajeExito"] = "El técnico fue actualizado correctamente.";
            return RedirectToAction(nameof(Detalle), new { id = servicio.Id });
        }
